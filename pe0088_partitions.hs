...
myWrap m = foldl rAdd 0 $sort $ map (\(a,b)->(b,a))$ foldl lEqFold [] $sort $ map (\(a,b)->(b,a)) $ reverse $foldl lEqFold [] $ sort $filter (\(s,u)-> s<=m && s>1) $ map score $ concat $ map partitionsUloPE (loInts (div (13*m) 12) )
